...
export default class Routes extends Component {
	...
	handleAdd = (title, path) => {
		RoutesDataService.add({ title: title, path: path })
			.then((response) => {
				...
				<div>
					<div className='d-inline-flex flex-column p-2'>
						...
						<button
							...
							onClick={() => this.handleAdd(this.state.title, this.state.path)}
							type='submit'
						>
							Добавить
						</button>
